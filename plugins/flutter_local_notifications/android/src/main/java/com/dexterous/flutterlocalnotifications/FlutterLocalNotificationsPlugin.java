...
public class FlutterLocalNotificationsPlugin
  ...
  protected static Notification createNotification(
    ...
    // أول إجراء مرئي — يُرسم كحبّة داخل البطاقة المخصّصة، لأن أزرار النظام
    // لا تُرسم مع إشعار مخصّص بلا DecoratedCustomViewStyle.
    final PendingIntent[] cardActionIntent = new PendingIntent[1];
    final CharSequence[] cardActionLabel = new CharSequence[1];
    if (notificationDetails.actions != null) {
      ...
      for (NotificationAction action : notificationDetails.actions) {
        ...
        if (cardActionIntent[0] == null && !BooleanUtils.getValue(action.invisible)) {
          cardActionIntent[0] = actionPendingIntent;
          cardActionLabel[0] = actionTitleSpannable;
        }
    ...
    setCustomNotificationLayout(
        context, notificationDetails, builder, cardActionIntent[0], cardActionLabel[0]);
  ...
  // ───────────────────────────────────────────────────────────────────────────
  //  بطاقة إشعار مخصّصة (RemoteViews) — الحل المضمون لتلوين خلفية الإشعار
  // ───────────────────────────────────────────────────────────────────────────

  /**
   * يبني بطاقة الإشعار من تخطيط RemoteViews مُعرَّف في وحدة التطبيق.
   *
   * لماذا هذا ضروري؟ لأن `setColorized(true)` وحده لا يغيّر خلفية الإشعار:
   * توثيق أندرويد يشترط وجود «custom content view» ليُطبَّق التلوين أصلاً. وبما
   * أن التخطيط يُنفَخ داخل عملية SystemUI، تُمرَّر كل القيم (اللون، النص،
   * الأيقونة) عبر RemoteViews لا عبر موارد ثابتة.
   *
   * وإن غاب المورد لأي سبب نتوقف بهدوء ونعود للقالب الافتراضي بدل تعطيل الإشعار.
   */
  private static void setCustomNotificationLayout(
      Context context,
      NotificationDetails notificationDetails,
      NotificationCompat.Builder builder,
    ...
    try {
      if (StringUtils.isNullOrEmpty(notificationDetails.customLayout)) {
        return;
      }
      int layoutId = getAppResourceId(context, notificationDetails.customLayout, "layout");
      if (layoutId == 0) {
        Log.e(TAG, "custom layout not found: " + notificationDetails.customLayout);
        return;
      }

      int cardColor =
          notificationDetails.color != null
              ? notificationDetails.color.intValue()
              : Color.parseColor("#FF002855");

      // ⚠️ يُمرَّر الإجراء إلى البطاقة **المطوية** أيضاً لا إلى الموسّعة وحدها:
      //    البطاقة المطوية هي الظاهرة أولاً وأثناء الظهور السريع (heads-up)،
      //    فلو جاءت بلا إجراء لما رأى المستخدم زرّ إيقاف الأذان إلا بعد سحب
      //    الإشعار وتوسيعه — وهو أسوأ وقت للبحث عن زرّ الإيقاف. والبطاقة تحلّ
      //    الحبّة **بدل** الوقت عند وجود إجراء (لا يتّسع الصفّ للاثنين).
      RemoteViews collapsed =
          inflateCustomCard(
              context, layoutId, notificationDetails, cardColor, false, actionIntent, actionLabel);
      if (collapsed == null) {
        return;
      }

      // ⚠️ بلا `DecoratedCustomViewStyle` عن قصد: ذلك النمط يبقي «زخرفة النظام»
      //    (اسم التطبيق والوقت وإطار البطاقة) ثم يضع بطاقتنا داخلها، فيظهر
      //    «إشعار صغير داخل إشعار كبير». بإلغائه تصبح بطاقتنا هي الإشعار كله.
      //    ونتيجة إلغائه أن أزرار النظام لا تُرسم — ولذلك ترسم البطاقة حبّة
      //    إجراء خاصة مربوطة بنفس الـ PendingIntent (انظر cardActionIntent).
      builder.setColorized(true);
      builder.setColor(cardColor);
      builder.setCustomContentView(collapsed);
      builder.setCustomHeadsUpContentView(collapsed);

      if (!StringUtils.isNullOrEmpty(notificationDetails.customBigLayout)) {
        int bigLayoutId = getAppResourceId(context, notificationDetails.customBigLayout, "layout");
        if (bigLayoutId != 0) {
      ...
      Log.e(TAG, "setCustomNotificationLayout error: " + e.getMessage());
    }
  }

  /**
   * يملأ تخطيط البطاقة بالقيم الفعلية للإشعار (لون، عنوان، نص، أيقونة، وقت).
   *
   * كل المعرّفات تُقرأ بالاسم من حزمة التطبيق، فالإضافة لا تعرف موارد التطبيق
   * وقت الترجمة.
   *
   * @param isExpanded هل نبني البطاقة الموسّعة؟ عنصرا التذييل (`notif_footer`
   *     و`notif_divider`) موجودان في التخطيط الموسّع فقط، واستهداف عنصر غير
   *     موجود داخل RemoteViews يُفسد الإشعار — فلا نلمسهما في البطاقة المطوية
   *     إطلاقاً. كما أن الافتراضي فيهما «مخفي»، فلا نضيف إجراءً بلا نص تذييل.
   */
  private static RemoteViews inflateCustomCard(
      Context context,
      int layoutId,
      NotificationDetails notificationDetails,
      int cardColor,
    ...
    try {
      ...
      // ① البطاقة الملوّنة (شكل أبيض دائري الزوايا يُلوَّن بمرشّح لون)
      int bgId = getAppResourceId(context, "notif_card_bg", "id");
      if (bgId != 0) {
        views.setInt(bgId, "setColorFilter", cardColor);
      }

      // ② القرص الدائري خلف الأيقونة — أبيض شفاف ليعطي إحساس الزجاج
      int discId = getAppResourceId(context, "notif_icon_badge", "id");
      if (discId != 0) {
        views.setInt(discId, "setColorFilter", Color.argb(38, 255, 255, 255));
      }

      // ③ شريط التمييز — ذهبي التطبيق افتراضاً، ويمكن تمرير لون آخر
      int accentId = getAppResourceId(context, "notif_accent", "id");
      if (accentId != 0) {
        views.setInt(
            accentId,
            "setColorFilter",
            notificationDetails.customLayoutAccent != null
                ? notificationDetails.customLayoutAccent.intValue()
                : Color.parseColor("#FFDFBA6B"));
      }

      // ④ الأيقونة داخل البطاقة
      int iconId = getAppResourceId(context, "notif_icon", "id");
      if (iconId != 0 && !StringUtils.isNullOrEmpty(notificationDetails.customLayoutIcon)) {
        int iconRes = getAppResourceId(context, notificationDetails.customLayoutIcon, "drawable");
        if (iconRes != 0) {
          views.setImageViewResource(iconId, iconRes);
        }
      }

      // ⑤ نصوص البطاقة — أهم عنصر في الوضوح
      int titleId = getAppResourceId(context, "notif_title", "id");
      if (titleId != 0) {
        if (StringUtils.isNullOrEmpty(notificationDetails.title)) {
          views.setViewVisibility(titleId, View.GONE);
        } else {
          views.setTextViewText(titleId, notificationDetails.title);
        }
      }

      int bodyId = getAppResourceId(context, "notif_body", "id");
      if (bodyId != 0) {
        if (StringUtils.isNullOrEmpty(notificationDetails.body)) {
          views.setViewVisibility(bodyId, View.GONE);
        } else {
          views.setTextViewText(bodyId, notificationDetails.body);
        }
      }

      int timeId = getAppResourceId(context, "notif_time", "id");
      if (timeId != 0) {
        if (StringUtils.isNullOrEmpty(notificationDetails.customLayoutBadge)) {
          views.setViewVisibility(timeId, View.GONE);
        } else {
          views.setTextViewText(timeId, notificationDetails.customLayoutBadge);
        }
      }

      boolean hasAction = actionIntent != null && actionLabel != null && actionLabel.length() > 0;

      if (isExpanded) {
        // ⑥ صفّ التذييل: يظهر عند وجود تذييل أو إجراء، وإلا يبقى مخفياً بالكامل
        boolean hasFooter = !StringUtils.isNullOrEmpty(notificationDetails.customLayoutFooter);
        if (hasFooter || hasAction) {
          int rowId = getAppResourceId(context, "notif_footer_row", "id");
          int dividerId = getAppResourceId(context, "notif_divider", "id");
          if (rowId != 0) {
            views.setViewVisibility(rowId, View.VISIBLE);
          }
          if (dividerId != 0) {
            views.setViewVisibility(dividerId, View.VISIBLE);
          }
          int footerId = getAppResourceId(context, "notif_footer", "id");
          if (hasFooter) {
            if (footerId != 0) {
              views.setViewVisibility(footerId, View.VISIBLE);
              views.setTextViewText(footerId, notificationDetails.customLayoutFooter);
            }
          } else if (hasAction && footerId != 0) {
            // بلا نصّ تذييل: يُحتفظ بمُعبِّئ المساحة (TextView بوزن 1) بنصّ فارغ كي
            // يبقى زرّ الإجراء في **نهاية** الصفّ (أقصى اليسار في RTL) بدل أن
            // ينزلق إلى وسطه حين يختفي ما كان يدفعه.
            views.setViewVisibility(footerId, View.VISIBLE);
            views.setTextViewText(footerId, "");
          }
        }
      } else if (hasAction && timeId != 0) {
        // ⑦ في الصفّ المطوي: حبّة الإجراء تحلّ مكان الوقت (لا يتّسع للاثنين)
        views.setViewVisibility(timeId, View.GONE);
      }

      if (hasAction) {
        int actionId = getAppResourceId(context, "notif_action", "id");
        if (actionId != 0) {
          views.setViewVisibility(actionId, View.VISIBLE);
          views.setTextViewText(actionId, actionLabel);
          views.setOnClickPendingIntent(actionId, actionIntent);
        }
      }

      // ⑧ دلالة للقارئ الشاشي: البطاقة المخصّصة تتجاهل النص الافتراضي للإشعار
      int contentId = getAppResourceId(context, "notif_content", "id");
      if (contentId != 0) {
        StringBuilder description = new StringBuilder();
        if (!StringUtils.isNullOrEmpty(notificationDetails.title)) {
          description.append(notificationDetails.title);
        }
        if (!StringUtils.isNullOrEmpty(notificationDetails.body)) {
          if (description.length() > 0) {
            description.append("، ");
          }
          description.append(notificationDetails.body);
        }
        if (!StringUtils.isNullOrEmpty(notificationDetails.customLayoutBadge)) {
          if (description.length() > 0) {
            description.append("، ");
          }
          description.append(notificationDetails.customLayoutBadge);
        }
        if (description.length() > 0) {
          views.setContentDescription(contentId, description.toString());
        }
      }

      return views;
    } catch (Exception e) {
      Log.e(TAG, "inflateCustomCard error: " + e.getMessage());
      return null;
    }
  }

  /**
   * قراءة مورد من حزمة التطبيق بالاسم وقت التشغيل — تُرجع 0 إن لم يوجد.
   *
   * التخطيطات تبقى في وحدة التطبيق (لا في الإضافة) لأن RemoteViews يشترط مورداً
   * من حزمة التطبيق ليُنفَخ داخل عملية SystemUI.
   */
  private static int getAppResourceId(Context context, String name, String type) {
    try {
