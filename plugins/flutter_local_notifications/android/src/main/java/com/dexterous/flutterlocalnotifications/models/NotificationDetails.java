...
public class NotificationDetails implements Serializable {
  ...
  // ── بطاقة إشعار مخصّصة (RemoteViews) ───────────────────────────────────────
  // تُعرَّف التخطيطات في وحدة التطبيق (android/app/src/main/res/layout) ويصل
  // إليها هذا الملف بالاسم عبر getIdentifier، فتستطيع الإضافة رسم بطاقة
  // إشعار كاملة بألوان التطبيق — وهو الحل الوحيد المضمون لتلوين خلفية الإشعار.
  private static final String CUSTOM_LAYOUT = "customLayout";
  private static final String CUSTOM_BIG_LAYOUT = "customBigLayout";
  private static final String CUSTOM_LAYOUT_ICON = "customLayoutIcon";
  private static final String CUSTOM_LAYOUT_BADGE = "customLayoutBadge";
  private static final String CUSTOM_LAYOUT_FOOTER = "customLayoutFooter";
  private static final String CUSTOM_LAYOUT_ACCENT_ALPHA = "customLayoutAccentAlpha";
  private static final String CUSTOM_LAYOUT_ACCENT_RED = "customLayoutAccentRed";
  private static final String CUSTOM_LAYOUT_ACCENT_GREEN = "customLayoutAccentGreen";
  private static final String CUSTOM_LAYOUT_ACCENT_BLUE = "customLayoutAccentBlue";
  ...
  // ── بطاقة إشعار مخصّصة (RemoteViews) ──
  /** اسم تخطيط البطاقة المطوية في وحدة التطبيق (مثال: notification_prayer_collapsed) */
  public String customLayout;
  /** اسم تخطيط البطاقة الموسّعة (اختياري) */
  public String customBigLayout;
  /** اسم drawable للأيقونة داخل البطاقة (اختياري) */
  public String customLayoutIcon;
  /** نص صغير في طرف البطاقة — يُستخدم لوقت الصلاة أو رمز اللون */
  public String customLayoutBadge;
  /** نص التذييل في البطاقة الموسّعة (اختياري) */
  public String customLayoutFooter;
  /** لون شريط التمييز داخل البطاقة (اختياري، افتراضياً ذهبي من التخطيط) */
  public Integer customLayoutAccent;
  ...
  private static void readPlatformSpecifics(
    ...
    if (platformChannelSpecifics != null) {
      ...
      readCustomLayout(notificationDetails, platformChannelSpecifics);
  ...
  /**
   * قراءة تفاصيل بطاقة الإشعار المخصّصة (إن مُرّرت).
   *
   * التخطيطات نفسها تُعرَّف في وحدة التطبيق لا في الإضافة، لأن RemoteViews يحتاج
   * مورداً من حزمة التطبيق ليُنفخ داخل عملية SystemUI.
   */
  private static void readCustomLayout(
      NotificationDetails notificationDetails, Map<String, Object> platformChannelSpecifics) {
    if (!platformChannelSpecifics.containsKey(CUSTOM_LAYOUT)) {
      return;
    }
    notificationDetails.customLayout = (String) platformChannelSpecifics.get(CUSTOM_LAYOUT);
    notificationDetails.customBigLayout =
        (String) platformChannelSpecifics.get(CUSTOM_BIG_LAYOUT);
    notificationDetails.customLayoutIcon =
        (String) platformChannelSpecifics.get(CUSTOM_LAYOUT_ICON);
    notificationDetails.customLayoutBadge =
        (String) platformChannelSpecifics.get(CUSTOM_LAYOUT_BADGE);
    notificationDetails.customLayoutFooter =
        (String) platformChannelSpecifics.get(CUSTOM_LAYOUT_FOOTER);
    Integer a = (Integer) platformChannelSpecifics.get(CUSTOM_LAYOUT_ACCENT_ALPHA);
    Integer r = (Integer) platformChannelSpecifics.get(CUSTOM_LAYOUT_ACCENT_RED);
    Integer g = (Integer) platformChannelSpecifics.get(CUSTOM_LAYOUT_ACCENT_GREEN);
    Integer b = (Integer) platformChannelSpecifics.get(CUSTOM_LAYOUT_ACCENT_BLUE);
    if (a != null && r != null && g != null && b != null) {
      notificationDetails.customLayoutAccent = Color.argb(a, r, g, b);
    }
  }
