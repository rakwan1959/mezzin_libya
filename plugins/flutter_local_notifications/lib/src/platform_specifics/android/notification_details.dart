...
class AndroidNotificationDetails {
  ...
  const AndroidNotificationDetails(
    ...
    this.customLayout,
    this.customBigLayout,
    this.customLayoutIcon,
    this.customLayoutBadge,
    this.customLayoutFooter,
    this.customLayoutAccent,
    this.number,
  ...
  /// اسم تخطيط `RemoteViews` في وحدة التطبيق لبطاقة الإشعار المطوية.
  ///
  /// هذا هو الحل الوحيد المضمون لتلوين **خلفية** الإشعار: توثيق أندرويد
  /// يشترط وجود «custom content view» ليُطبَّق التلوين، وإلا بقي [color] لون
  /// تمييز للأيقونة واسم التطبيق فقط. يُقرأ المورد بالاسم وقت التشغيل من حزمة
  /// التطبيق، لذا يجب حمايته من تقليص الموارد (انظر `res/raw/keep.xml`).
  ///
  /// رمزياً: `'notification_prayer_collapsed'`
  final String? customLayout;

  /// اسم تخطيط البطاقة الموسّعة — تظهر عند سحب الإشعار لأسفل.
  final String? customBigLayout;

  /// اسم `drawable` للأيقونة المرسومة داخل البطاقة.
  final String? customLayoutIcon;

  /// نص صغير في طرف البطاقة (مثل وقت الصلاة أو رمز اللون الست عشري).
  ///
  /// يُستخدم للتحقّق الدقيق من اللون أثناء التجربة. إن لم يُمرَّر يُخفى العنصر.
  final String? customLayoutBadge;

  /// نص التذييل في البطاقة الموسّعة — يظهر معه فاصل رقيق تلقائياً.
  final String? customLayoutFooter;

  /// لون شريط التمييز داخل البطاقة (افتراضياً ذهبي التطبيق).
  final Color? customLayoutAccent;
